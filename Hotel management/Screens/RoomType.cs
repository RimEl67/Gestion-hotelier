...
namespace Hotel_Management_System.Screens
{
    public partial class RoomType : Form
    {
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(typeIdField.Text))
            {
                MessageBox.Show("Please enter a valid RoomType ID to delete.", "Missing Info", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            // Validez si l'entrée est un nombre entier valide
            if (!int.TryParse(typeIdField.Text, out int roomTypeId))
            {
                MessageBox.Show("Please enter a valid numeric RoomType ID.", "Invalid Input", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // Vérifiez les dépendances avant de supprimer
            string checkQuery = "SELECT COUNT(*) FROM Rooms.Room WHERE RoomTypeId = @RoomTypeId";
            using (SqlConnection con = dc.getConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand(checkQuery, con))
                {
                    ...
                    if (count > 0)
                    {
                        MessageBox.Show("This RoomType is associated with existing rooms. Please delete the rooms first or handle dependencies.", "Cannot Delete", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                }

                // Si aucune dépendance, procédez à la suppression
                string deleteQuery = "DELETE FROM Rooms.RoomType WHERE RoomTypeId = @RoomTypeId";
                using (SqlCommand deleteCmd = new SqlCommand(deleteQuery, con))
                {
                    deleteCmd.Parameters.AddWithValue("@RoomTypeId", roomTypeId);
                    ...
                    {
                        MessageBox.Show("No record found with the specified RoomType ID.", "Not Found", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
        }

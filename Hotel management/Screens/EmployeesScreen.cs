...
namespace Hotel_Management_System.Controllers
{
    public partial class EmployeesScreen : Form
    {
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter a valid Employee ID to delete.", "Missing Info", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            // Validation de l'entrée
            if (!int.TryParse(employeeIdField.Text, out int employeeId))
            {
                MessageBox.Show("Please enter a valid numeric Employee ID.", "Invalid Input", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            using (SqlConnection con = dc.getConnection())
            {
                ...
                // Vérifiez si l'employé est référencé dans des réservations
                string checkQuery = "SELECT COUNT(*) FROM Bookings.Booking WHERE EmployeeId = @EmployeeId";
                using (SqlCommand checkCmd = new SqlCommand(checkQuery, con))
                {
                    ...
                    if (count > 0)
                    {
                        MessageBox.Show("This employee is associated with existing bookings. Please reassign or delete the bookings first.", "Cannot Delete", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                }

                // Supprimez l'employé s'il n'est pas référencé
                string deleteQuery = "DELETE FROM Hotels.Employees WHERE EmployeeId = @EmployeeId";
                using (SqlCommand deleteCmd = new SqlCommand(deleteQuery, con))
                {
                    ...
                    {
                        MessageBox.Show("No employee found with the specified ID.", "Not Found", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
        }
